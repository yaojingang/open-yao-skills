...
- **高密度图表排版**：树图、热力图和横向条形图会显式占满容器，并通过标签折行、隐藏小块标签和容器 resize 避免右侧留白与文字裁切。
- **读者画像升级**：报告顶部生成一段温暖的诗性总结和 1 条画像金句，图表下方列出最多 20 条与阅读偏好、词云关键词和笔记特征相关的高价值划线。
...
- 画像金句与 20 条划线清单只从 `/book/bookmarklist` 返回的用户划线中筛选；顶部诗性总结基于统计数据生成，不冒充原始划线。
...
- `weread-report-data.json`: 图表数据、画像划线和聚合结构
- `weread-raw-summary.json`: 覆盖范围、关键 KPI、图表数量和画像划线数量
